...
| # | Decision | Resolution |
|---|---|---|
| D1 | `__VERSION__` string form | **Bare, no `v` prefix** — `1.55.3` for this build — matching `package.json` and the CLI banner. The value is read from the existing version source, never restated as a fourth location. The doc's `'v1.43.0'` example is corrected, not the value. |
...
Generated by `plan-to-tasks` at sprint start. Sprint tag: `preproc-symbols`.
`seq` is the authoritative implementation order.

| Plan § | Deliverable | Task | seq | Est |
| ------ | ----------- | ---- | --- | --- |
| §10 A+B | External audit findings — fix 4, document 2 | «#15» | 1 | 45m |
| §1 | `#undef` must clear the substitution table | «#16» | 2 | 75m |
| §5 | `#undef` of a built-in warns and refuses | «#17» | 3 | 60m |
| §2 | Function-like `#define` is an error | «#18» | 4 | 75m |
| §3 | Multi-word `#define` values | «#19» | 5 | 75m |
| §4 | `__VERSION__` visible to the preprocessor | «#20» | 6 | 60m |
| §8 + §10 C | Language-spec pipeline: assess, then act | «#21» | 7 | 120m |
| §9 | `copyright` reconcile and ship | «#22» | 8 | 30m |
| §7 | `SPIN2-BIN-Format.md` verify against v54/v55 | «#23» | 9 | 90m |
| §6 | `Preprocessor.md` symbols table + full verify | «#24» | 10 | 90m |
| §11 | Documentation, release notes, version bump | «#25» | 11 | 90m |

**Splits and groupings.** §10 is the only section split, and it splits by
*disposition* rather than by size: Groups A and B (fix four, document two) are
«#15», while Group C — the five findings inside
`DOCs/language-specification/extraction-scripts/` — is folded into «#21», because
those files sit in the tree §8 assesses and may be rewritten or retired by its
outcome. Fixing them independently would risk polishing files that Phase A
condemns. No sections were grouped.

**Ordering rationale (§3a rework pass).**

- **«#15» first for a file-collision reason**, not importance: it edits
  `src/pnut-ts.ts:102` while §4 «#20» edits `src/pnut-ts.ts:588`. Doing the
  audit sweep first keeps the two changes out of each other's way.
- **Foundation before building** — «#16» → «#17» (both edit `undefineSymbol()`;
  the built-in guard depends on the both-tables contract) and «#18» → «#19»
  (both edit `getSymbolValue()`; the symbol-shape validation is what makes
  liberal value parsing safe).
- **Discovery before utilization** — «#21» Phase A must answer whether the
  pipeline runs before anything touches the extraction scripts, which is
  precisely why Group C rides along inside it.
- **Audit at its natural gate** — the documentation verifications «#24» and
  «#23» sit *after* the code they describe, so they document shipped behavior
  rather than planned behavior. «#25» last, as it bumps the version and writes
  the release notes.
- **Generated artifacts** — «#21» is the only task that regenerates anything,
  and nothing else in this sprint consumes its outputs, so no
  regenerate-then-consume ordering constraint applies.

**Atomic green-unit check** (per `.claude/skills/plan-to-tasks/project-overlay.md`):
**none found.** Each task reaches a protection point on its own. «#16», «#18»
and «#19» may each turn a fixture red — a source relying on post-`#undef`
substitution, a dead function-like macro, a GOLD encoding the truncation bug —
but in every case that red is a **finding to read**, not a transient state a
later task repairs. Unlike CLI-Robustness §1/§2, no task here surfaces a defect
that another task fixes. If execution proves otherwise, that is a re-tasking
signal per §6 of the skill.
