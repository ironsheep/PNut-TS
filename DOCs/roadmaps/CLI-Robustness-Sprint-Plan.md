...
- **Order.** Settled by the `plan-to-tasks` rework-analysis pass; the `seq`
  column in the cross-reference table below is authoritative.
  1. **§11** first — it unblocks trustworthy `.errout` capture for
     everything downstream.
  2. **§7** second. *This corrects the plan's original claim that §7 "is
     independent and can land at any point."* The rework pass caught it:
     §7 establishes the exact stderr format (single plain line, no ANSI)
     that every `.errout.GOLD` in §9b encodes. Authoring fixtures first
     would force re-recording all of them — the "standards before
     application" conflict pattern.
  3. **§1** — the severity mechanism §2–§6 all build on.
  4. **§2 → §3 → §4 → §5 → §6** — the diagnostics themselves, which supply
     the message text the fixtures assert.
  5. **§8** — independent of §1–§6; ordered here so §9a/§9d can assert
     against real delete-on-failure behavior rather than stubs.
  6. **§9a-c**, then **§9d** — fixtures last among code work, since they
     encode everything above.
  7. **§12** — after §11 (else the default suite goes red on the Group A
     artifact) and after §9 (else red on fixtures not yet written).
  8. **§10** — documentation last, verified against shipped behavior.
...
Generated by `plan-to-tasks` at sprint start. Sprint tag: `cli-robustness`.
`seq` is the authoritative implementation order (see § Notes for execution).

| Plan § | Deliverable | Task | seq | Est |
| ------ | ----------- | ---- | --- | --- |
| §11 | Test-harness stderr pollution | «#2» | 1 | 60m |
| §7 | Single plain-text error output | «#3» | 2 | 45m |
| §1 | Preprocessor diagnostic infrastructure | «#4» | 3 | 120m |
| §2 | `#error` / `#warn` correctness | «#5» | 4 | 60m |
| §3 | Malformed and bare directives | «#6» | 5 | 90m |
| §4 | Classify remaining directive diagnostics | «#7» | 6 | 75m |
| §5 | Spin2 language-version diagnostic | «#8» | 7 | 60m |
| §6 | Surface `#include` argument diagnostics | «#9» | 8 | 30m |
| §8 | Delete output artifacts on failure | «#10» | 9 | 120m |
| §9a-c | Test scaffolding + preprocessor/CLI fixtures | «#11» | 10 | 180m |
| §9d | New artifact-cleanup test suite | «#12» | 11 | 90m |
| §12 | Default-suite coverage for the test homes | «#13» | 12 | 30m |
| §10 | Documentation | «#14» | 13 | 90m |

**Splits and groupings.** §9 is the only section split — into «#11»
(scaffolding + fixtures) and «#12» (the new CLEANUP suite) — because each
reaches a green protection point independently and they exercise different
subjects. No sections were grouped: each of the other eleven is a complete
deliverable that ends green on its own.
