...
Propeller Spin2 compiler - v1.55.7
...
| <pre>-m, --map</pre> | generate a memory map file (.map) naming each compiled *image* (the code and DAT one or more objects compile to) and each *instance* (the top object, or one `OBJ` declaration or array element beneath it), with the address range, size and VAR block of every instance and every image (as of v1.55.8 — see `DOCs/internals/MAP-File-Format.md` for the format and the CHANGELOG if you parse these files) |
